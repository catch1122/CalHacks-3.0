    ...
    public class MenuButton : MonoBehaviour
    {
      ...
      //  [SerializeField] private string m_SceneToLoad;                      // The name of the scene to load.
        ...
        private IEnumerator ActivateButton()
        {
			...
			//yield return StartCoroutine(0);
			foreach (Transform trans in GameObject.Find ("Sphere").GetComponentInChildren<Transform>()) {
				trans.gameObject.SetActive (true);
			}
			GameObject.Find ("Stuff").SetActive (false);
			GameObject.Find ("MainCamera").GetComponent<ReturnToMainMenu> ().m_MenuSceneName = "Location Menu";
			GameObject photo = GameObject.Find ("Photo");
			string url = GetComponent<SetLocationImage> ().url;
			Debug.Log ("photo url: " + url);
			if (photo != null) {
				Debug.Log ("Photo");
				photo.GetComponent<SetLocationImage> ().SetImage (url);
				photo.GetComponent<Renderer> ().material.mainTexture = GetComponent<Renderer> ().material.mainTexture;
			//	Material mat = GetComponent<Material> ();
			//	photo.GetComponent<Material> () = mat;
			}

			//GameObject.Find ("Photo").GetComponent<SetLocationImage> ().SetImage (GetComponent<SetLocationImage> ().url);
